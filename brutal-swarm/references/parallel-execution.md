...
New launches use the content-addressed manifest v3 contract in
`compositional-harness.md`; retained v2 attempts continue unchanged.
...
| `reconcile` | mechanically resolve a returned conflict | checkpoint | originating `work` or `fix` |
...
`failed`. Accept `conflict` only from v3 `work` or `fix`. Treat nonzero exit,
missing/partial exit, malformed result, wrong task/phase/attempt/context, or
stale digest as failure. The controller never accepts a worker-supplied next
phase.
