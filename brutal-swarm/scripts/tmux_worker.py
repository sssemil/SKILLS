...
_PHASES = ("work", "review", "fix", "reconcile", "handoff", "complete")
_CHECKPOINT_PHASES = ("work", "review", "fix", "reconcile")
...
_CONFLICT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "conflict_artifact_digest": {"type": "string", "pattern": r"^[0-9a-f]{64}$"},
    },
}
...
def _managed_result_schema(
    phase: str, *, managed_version: int = 2, context_digest: str | None = None
) -> dict[str, Any]:
    ...
    if phase in _CHECKPOINT_PHASES:
        ...
        if phase in {"work", "fix"} and managed_version == 3:
            variants.append(
                _managed_phase_variant(phase, "conflict", {"conflict": _CONFLICT_SCHEMA})
            )
    else:
        completion_kinds = (
    ...
    if managed_version == 3:
        if context_digest is None:
            raise TmuxWorkerError("managed v3 result schema requires context digest")
        for variant in schema["oneOf"]:
            variant["properties"]["schema_version"] = {"const": 3}
            variant["properties"]["context_digest"] = {"const": context_digest}
            variant["required"].append("context_digest")
    return schema
...
def _coordination_paths(
    state_dir: Path, handoff: Mapping[str, Any]
) -> tuple[Path, Path]:
    plan_identity = str(handoff.get("plan_ref") or handoff.get("run_id") or "default")
    plan_digest = hashlib.sha256(plan_identity.encode("utf-8")).hexdigest()[:16]
    root = state_dir.parent / "coordination"
    return root / "plans" / plan_digest / "decision-registry.json", root / "field-guide.json"


def _install_decision_registry(state_dir: Path, handoff: Mapping[str, Any]) -> dict[str, str]:
    registry_path, guide_path = _coordination_paths(state_dir, handoff)
    try:
        registry = normalize_decision_registry(handoff.get("decision_registry", []))
    except ContextError as exc:
        raise TmuxWorkerError(f"invalid managed decision registry: {exc}") from exc
    value = {"schema_version": 1, "decisions": registry}
    existing = _read_json(registry_path)
    if existing is not None and existing != value:
        raise TmuxWorkerError("plan decision registry is immutable")
    if existing is None:
        _atomic_json(registry_path, value)
    return {
        "decision_registry": str(registry_path),
        "field_guide": str(guide_path),
    }
...
def _task_capsule(manifest: Mapping[str, Any]) -> dict[str, Any]:
    handoff = manifest["handoff"]
    assert isinstance(handoff, Mapping)
    supplied = handoff.get("task_capsule")
    if isinstance(supplied, Mapping):
        return build_task_capsule(supplied)
    task_ref = str(manifest["task_ref"])
    return build_task_capsule(
        {
            "task": {
                "ref": task_ref,
                "kind": str(handoff.get("task_kind") or "task"),
                "title": str(handoff.get("task_title") or task_ref),
                "goal": str(handoff.get("task_goal") or f"Complete {task_ref}"),
                "acceptance_criteria": list(handoff.get("acceptance_criteria") or []),
                "blockers": list(handoff.get("blockers") or []),
            },
            "coordination": handoff.get("coordination"),
            "verification_commands": list(handoff.get("verification_commands") or []),
...
def _phase_projections(
    manifest: Mapping[str, Any], phase: str, live: Mapping[str, Any]
) -> dict[str, Any]:
    handoff = manifest["handoff"]
    assert isinstance(handoff, Mapping)
    if phase == "work":
        coordination_paths = manifest.get("coordination")
        projections: dict[str, Any] = {}
        capsule = _task_capsule(manifest)
        if isinstance(coordination_paths, Mapping):
            registry = _read_json(Path(str(coordination_paths["decision_registry"])))
            if registry is not None:
                wanted = {
                    item["id"]
                    for item in capsule["coordination"]["decisions_owned"]
                } | set(capsule["coordination"]["decisions_consumed"])
                projections["decision_registry"] = {
                    "schema_version": 1,
                    "decisions": [
                        item
                        for item in registry.get("decisions", [])
                        if isinstance(item, Mapping) and item.get("id") in wanted
                    ],
                }
            guide_path = Path(str(coordination_paths["field_guide"]))
            touch_paths = [
                surface["path"]
                for surface in capsule["coordination"]["touch_surfaces"]
            ]
            with field_guide_lock(guide_path):
                guide = load_field_guide(guide_path)
                relevant = project_field_guide(guide, touch_paths=touch_paths)
                guide = touch_field_guide(
                    guide_path, [entry["id"] for entry in relevant["entries"]]
                )
                projections["field_guide"] = project_field_guide(
                    guide, touch_paths=touch_paths
                )
        if "repository_rules" in handoff:
            projections["repository_rules"] = handoff["repository_rules"]
        return projections
    if phase == "review":
        raw = handoff.get("review_context")
        if not isinstance(raw, Mapping):
            raw = {}
        return {
            f"review_lens_{lens}": review_lens_projection(lens, raw)
            for lens in (
                "product",
                "correctness",
                "reliability",
    ...
    if phase == "fix":
        return {"finding_queue": handoff.get("finding_queue", [])}
    if phase == "reconcile":
        return {"conflict": live.get("conflict")}
    return {
        key: handoff[key]
        for key in ("checkpoint_summary", "provider_summary")
        if key in handoff
    }
...
def _write_attempt(
    ...
    *,
    ...
    if managed_version == 3:
        context_manifest, context_digest = build_phase_manifest(
            state_dir,
            phase=phase,
            task_capsule=_task_capsule(manifest),
            phase_snapshot=snapshot,
            projections=_phase_projections(manifest, phase, live),
            outputs={"result": "result.json", "guide_proposals": "guide-proposals.json"},
        )
        context_path = attempt / "context-manifest.json"
        _atomic_json(context_path, context_manifest)
        prompt = build_phase_prompt(phase, context_path, context_digest)
    else:
    ...
    _atomic_json(
        ...
        _managed_result_schema(
            phase, managed_version=managed_version, context_digest=context_digest
        ),
    )
    _atomic_json(
...
def _read_result(
    ...
    *,
    ...
    if value.get("schema_version") != managed_version or status == "progress":
        return None
    if status not in {"checkpoint", "conflict", *_TERMINAL_STATUSES}:
        return None
    ...
    if managed_version == 3:
        if (
            expected_context_digest is None
            or value.get("context_digest") != expected_context_digest
        ):
            return None
        common.add("context_digest")
    if status == "checkpoint":
        ...
        if (
            managed_version != 3
            or value.get("phase") not in {"work", "fix"}
            or set(value) != common | {"conflict"}
            or not isinstance(conflict, Mapping)
            or set(conflict) != required
            or conflict.get("origin_phase") != value.get("phase")
            or not all(
                isinstance(conflict.get(key), str) and conflict.get(key)
                for key in ("operation", "base_sha", "head_sha")
            )
            or not isinstance(conflict.get("conflicted_paths"), list)
            or not conflict["conflicted_paths"]
            or not all(
                isinstance(path, str) and path for path in conflict["conflicted_paths"]
            )
            or not isinstance(conflict.get("conflict_artifact_digest"), str)
            or not re.fullmatch(r"[0-9a-f]{64}", conflict["conflict_artifact_digest"])
        ):
            return None
    elif status == "clean":
        if (
...
def launch_worker(
    *,
    ...
    os.chmod(state_dir, 0o700)
    if handoff is not None:
        manifest["coordination"] = _install_decision_registry(
            state_dir, resolved_handoff
        )
    _atomic_json(state_dir / "manifest.json", manifest)
    ...
    *,
    ...
    result = _read_result(
        ...
        expected_context_digest=(
            str(attempt_metadata.get("context_digest"))
            if attempt_metadata and attempt_metadata.get("context_digest")
            else None
        ),
    )
    ...
    response: dict[str, Any] = {
        ...
        "transition_digest": (
            _json_digest(result)
            if result is not None and result.get("status") in {"checkpoint", "conflict"}
            else None
        ),
        "context_digest": (
            attempt_metadata.get("context_digest") if attempt_metadata else None
        ),
        **_observer_commands(tmux_executable, tmux_socket, session),
    }
    if not exists:
    ...
    if process_failed:
        ...
        and result.get("status") in {"checkpoint", "conflict"}
...
def _derived_successor(
    result: Mapping[str, Any], active: Mapping[str, Any] | None = None
) -> str:
    phase = result.get("phase")
    if result.get("status") == "conflict":
        if phase not in {"work", "fix"}:
            raise TmuxWorkerError("only work or fix may request reconciliation")
        return "reconcile"
    if phase == "reconcile":
        return_phase = active.get("return_phase") if active else None
        if return_phase not in {"work", "fix"}:
            raise TmuxWorkerError("reconcile attempt has no valid return phase")
        return str(return_phase)
    if phase == "work":
        return "review"
    if phase == "fix":
...
def _attempt_result(
    ...
    result = _read_result(
        ...
        managed=True,
        managed_version=int(manifest["version"]),
        expected_context_digest=metadata.get("context_digest"),
    )
    if result is None or result.get("status") not in {"checkpoint", "conflict"}:
        raise TmuxWorkerError(
            "active attempt does not have an advanceable checkpoint or conflict transition"
        )
    digest = _json_digest(result)
    return result, exit_data, digest


def _merge_attempt_guide_proposals(
    state_dir: Path,
    manifest: Mapping[str, Any],
    active: Mapping[str, Any],
    worktree: Path,
    ...
    attempt = _attempt_dir(state_dir, str(active["attempt_id"]))
    proposal_path = attempt / "guide-proposals.json"
    marker_path = attempt / "guide-merged.json"
    if not proposal_path.exists() or marker_path.exists():
        return
    try:
        raw = json.loads(proposal_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise TmuxWorkerError(f"invalid Field Guide proposals: {exc}") from exc
    proposals = raw.get("proposals") if isinstance(raw, Mapping) else raw
    if not isinstance(proposals, list):
        raise TmuxWorkerError("Field Guide proposals must be an array")
    evidence_paths = {
        evidence.get("path")
    ...
    for evidence_path in sorted(evidence_paths):
        result = _git(worktree, "rev-parse", f"HEAD:{evidence_path}", check=False)
        if result.returncode == 0 and result.stdout.strip():
            blob_shas[evidence_path] = result.stdout.strip()
    coordination = manifest.get("coordination")
    if not isinstance(coordination, Mapping):
        raise TmuxWorkerError("managed Field Guide path is missing")
    try:
        guide_path = Path(str(coordination["field_guide"]))
        with field_guide_lock(guide_path):
            guide = merge_field_guide(
                guide_path,
                proposals,
                current_blob_shas=blob_shas,
            )
    except ContextError as exc:
        raise TmuxWorkerError(f"invalid Field Guide proposals: {exc}") from exc
    _atomic_json(
        marker_path,
        {"guide_revision": guide["revision"], "guide_digest": digest_json(guide)},
    )
    ...
    *,
    ...
    if resume_instruction:
        ...
        if manifest["version"] == 3:
            context_path = attempt / "context-manifest.json"
            context_manifest = _read_json(context_path)
            if context_manifest is None:
                raise TmuxWorkerError("managed v3 context manifest is missing")
            projections = context_manifest.get("projections")
            if not isinstance(projections, dict):
                raise TmuxWorkerError("managed v3 projections are malformed")
            projections["resume_instruction"] = store_artifact(
                state_dir, resume_instruction.strip(), "text/plain"
            )
            context_digest = digest_json(context_manifest)
            _atomic_json(context_path, context_manifest)
            prompt_path.write_text(
                build_phase_prompt(phase, context_path, context_digest),
                encoding="utf-8",
            )
            metadata = _read_json(attempt / "attempt.json") or {}
            ...
            _atomic_json(
                attempt / "result-schema.json",
                _managed_result_schema(
                    phase, managed_version=3, context_digest=context_digest
                ),
            )
        else:
            prompt_path.write_text(
...
def advance_worker(
    *,
    ...
    with _transition_lock(state_dir):
        ...
        _merge_attempt_guide_proposals(state_dir, manifest, active, worktree)
        phase = _derived_successor(result, active)
        return_phase = (
            str(result["phase"])
            if result.get("status") == "conflict"
            else None
        ...
        if result.get("status") == "conflict":
            live = {**live, "conflict": result["conflict"]}
    ...
    *,
    ...
    with _transition_lock(state_dir):
        ...
        result = _read_result(
            ...
            managed=True,
            managed_version=int(manifest["version"]),
            expected_context_digest=(_read_json(attempt / "attempt.json") or {}).get(
                "context_digest"
            ),
        )
        if (
        ...
        attempt_id, snapshot = _start_managed_attempt(
            ...
            return_phase=active.get("return_phase"),
        )
    return {
        "action": "resumed",
        "session_name": session,
    ...
    _atomic_json(
        runtime_dir / "exit.json",
        {
            ...
            "structured_result_valid": _read_result(
                ...
                managed_version=int(manifest["version"]) if managed else 2,
                expected_context_digest=(
                    attempt.get("context_digest") if managed and attempt else None
                ),
            )
            is not None,
        },
