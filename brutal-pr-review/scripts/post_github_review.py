...
unsafe inline mappings, patches only owned generated findings, posts every
finding as an independently actionable record, and creates append-only review
summaries. Live posting is the default; pass --dry-run to preview without
writing.
"""
...
TOOL = "brutal-pr-review"
SUMMARY_PREFIX = f"<!-- {TOOL}:summary:v2:"
FINGERPRINT_PREFIX = f"<!-- {TOOL}:v2:"
OCCURRENCE_PREFIX = f"<!-- {TOOL}-occurrence:"
...
def main() -> int:
    ...
    try:
        ...
        repo, pr_number, head_sha, review_id, summary, findings, inline_severities = validate_payload(payload)
        require_gh()
        ...
        execute_actions(actions, allow_fork=args.allow_fork)
...
def validate_payload(payload: Any) -> tuple[str, int, str, str, str, list[Finding], set[str]]:
    ...
    review_id = require_safe_token(payload, "review_id")
    if not SAFE_TOKEN_RE.fullmatch(head_sha):
        raise ReviewError("head_sha contains unsafe marker characters")
    ...
    for index, raw in enumerate(raw_findings):
        ...
        if not SAFE_TOKEN_RE.fullmatch(fingerprint):
            raise ReviewError(
    ...
    return repo, pr_number, head_sha, review_id, summary, findings, inline_severities
...
def require_safe_token(payload: dict[str, Any], key: str) -> str:
    value = require_str(payload, key)
    if not SAFE_TOKEN_RE.fullmatch(value):
        raise ReviewError(f"{key} contains unsafe marker characters")
    return value
...
def plan_actions(
    ...
    existing_inline = collect_existing_findings(review_comments, author_login, head_sha=head_sha)
    existing_top_level = collect_existing_findings(issue_comments, author_login)

    inline_actions = []
    top_level_actions = []
    for finding in findings:
        if finding.severity not in inline_severities:
            continue
        placement_reason = finding.fallback_reason
        if finding.can_post_inline(inline_severities):
            invalid_reason = invalid_inline_reason(finding, diff_index)
            if not invalid_reason:
                body = render_finding_body(finding, review_id, head_sha)
                ...
                continue
            placement_reason = invalid_reason

        placement_reason = placement_reason or "not eligible for an inline comment"
        body = render_finding_body(finding, review_id, head_sha, placement_reason)
        existing = existing_top_level.get(finding.marker)
        if existing:
            top_level_actions.append(
                {
                    "action": "patch_top_level",
                    "comment_id": existing["id"],
    ...
    return {
        ...
        "top_level": top_level_actions,
        "validated_finding_count": len(findings),
        "queued_finding_count": len(inline_actions) + len(top_level_actions),
        "findings_by_severity": finding_counts(findings),
    }


def collect_existing_findings(
    comments: list[Any], author_login: str, *, head_sha: str | None = None
    ...
    for comment in comments:
        ...
        if not marker:
            continue
        if head_sha is not None and comment.get("commit_id") != head_sha:
            continue
...
def render_finding_body(
    finding: Finding, review_id: str, head_sha: str, placement_reason: str | None = None
) -> str:
    parts = [
        finding.marker,
        f"{OCCURRENCE_PREFIX}{review_id}:{head_sha} -->",
        f"**Severity:** {finding.severity}",
        finding.body,
    ]
    if placement_reason:
        parts.append(f"Placement: top-level because {placement_reason}.")
    return trim_comment("\n\n".join(parts))


def finding_counts(findings: list[Finding]) -> dict[str, int]:
    return {severity: sum(finding.severity == severity for finding in findings) for severity in sorted(VALID_SEVERITIES)}
...
def build_summary_body(
    ...
    count_text = ", ".join(f"{severity}: {count}" for severity, count in counts.items())
    parts = [
        f"{SUMMARY_PREFIX}{review_id} -->",
        summary.strip(),
        f"Reviewed head: `{head_sha}`",
        f"Validated findings: {len(findings)} ({count_text})",
        f"Queued findings: {inline_count + top_level_count}",
        f"Placement: {inline_count} inline, {top_level_count} top-level",
    ]
    return trim_comment("\n\n".join(parts))
...
def trim_comment(body: str) -> str:
    if len(body) <= MAX_COMMENT_LENGTH:
        return body
    suffix = "\n\n[Truncated by brutal-pr-review because the GitHub comment limit was reached.]"
    return body[: MAX_COMMENT_LENGTH - len(suffix)] + suffix


def execute_actions(actions: dict[str, Any], *, allow_fork: bool) -> None:
    for action in actions["inline"]:
        verify_current_head(actions, allow_fork=allow_fork)
        apply_action(action)
...
def verify_current_head(actions: dict[str, Any], *, allow_fork: bool) -> None:
    latest_pr = gh_json(["api", f"repos/{actions['repo']}/pulls/{actions['pr_number']}"])
    verify_pr(actions["repo"], actions["head_sha"], latest_pr, allow_fork=allow_fork)
...
def apply_action(action: dict[str, Any]) -> None:
    ...
    elif action["action"] in {"create_summary", "create_top_level"}:
...
def summarize_actions(actions: dict[str, Any], *, status: str) -> dict[str, Any]:
    return {
        ...
        "review_id": actions["review_id"],
        "validated_finding_count": actions["validated_finding_count"],
        "queued_finding_count": actions["queued_finding_count"],
        "findings_by_severity": actions["findings_by_severity"],
        "summary": {
        ...
        "top_level_count": len(actions["top_level"]),
    }
