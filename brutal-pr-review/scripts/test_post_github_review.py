...
class PostGithubReviewTests(unittest.TestCase):
    def test_normal_mode_counts_but_does_not_queue_minor(self):
        with fake_gh() as env:
            ...
            self.assertEqual(planned["top_level"], [])
            self.assertEqual(planned["validated_finding_count"], 2)
            self.assertEqual(planned["queued_finding_count"], 1)
            self.assertIn("Queued findings: 1", planned["summary"]["body"])
            self.assertNotIn("POST", " ".join(" ".join(call) for call in read_calls(env)))

    def test_minor_posts_inline_when_inline_severities_opt_in(self):
        with fake_gh() as env:
            payload = base_payload(findings=[inline_finding("minor-1", severity="MINOR")])
            payload["inline_severities"] = ["CRITICAL", "MAJOR", "MINOR", "NIT"]
            result = run_script(payload, env, "--dry-run")
            self.assertEqual(result.returncode, 0, result.stderr)
            planned = json.loads(result.stdout)
            self.assertEqual(planned["inline"][0]["action"], "create_inline")
            self.assertEqual(planned["inline"][0]["fingerprint"], "minor-1")
            self.assertIn("**Severity:** MINOR", planned["inline"][0]["body"])
            self.assertEqual(planned["top_level"], [])

    def test_all_severities_are_counted_and_postable_inline(self):
        with fake_gh() as env:
            payload = base_payload(
                findings=[
                    inline_finding("critical", severity="CRITICAL"),
                    inline_finding("major", severity="MAJOR"),
                    inline_finding("minor", severity="MINOR"),
                    inline_finding("nit", severity="NIT"),
                ]
            )
            payload["inline_severities"] = ["CRITICAL", "MAJOR", "MINOR", "NIT"]
            result = run_script(payload, env, "--dry-run")
            self.assertEqual(result.returncode, 0, result.stderr)
            planned = json.loads(result.stdout)
            self.assertEqual(planned["validated_finding_count"], 4)
            self.assertEqual(len(planned["inline"]), 4)
            self.assertEqual(planned["findings_by_severity"], {
                "CRITICAL": 1,
                "MAJOR": 1,
                "MINOR": 1,
                "NIT": 1,
    ...
    def test_invalid_inline_severities_fail(self):
        with fake_gh() as env:
            payload = base_payload()
            payload["inline_severities"] = ["CRITICAL", "STYLE"]
            result = run_script(payload, env, "--dry-run")
            self.assertEqual(result.returncode, 2)
            self.assertIn("invalid inline severity", result.stderr)
    ...
    def test_live_mode_posts_by_default_and_reports_structured_result(self):
        with fake_gh() as env:
            ...
            self.assertEqual(posted["top_level_count"], 0)
            self.assertEqual(posted["validated_finding_count"], 1)
            self.assertEqual(posted["queued_finding_count"], 1)
    ...
    def test_live_mode_posts_non_line_mappable_finding_as_top_level(self):
        with fake_gh() as env:
            payload = base_payload(
                findings=[
                    {
                        ...
                        "severity": "MAJOR",
                        "body": "Cross-file architecture issue.",
                        "path": None,
                        "line": None,
                        "side": None,
                        "start_line": None,
                        "start_side": None,
                        "fallback_reason": "not line-mappable",
                    }
                ]
            )
            result = run_script(payload, env)
            self.assertEqual(result.returncode, 0, result.stderr)
            posted = json.loads(result.stdout)
            self.assertEqual(posted["inline_count"], 0)
            self.assertEqual(posted["top_level_count"], 1)
            flat = [" ".join(call) for call in read_calls(env)]
            self.assertEqual(
                sum("POST repos/O/R/issues/1/comments" in call for call in flat),
                2,
            )
    ...
    def test_posts_new_summary_and_patches_existing_inline_comment(self):
        with fake_gh(
            issue_comments=[
                generated_comment(10, "<!-- brutal-pr-review:summary:v2:old -->\nold")
            ],
            review_comments=[
                generated_comment(
                    20,
                    "<!-- brutal-pr-review:v2:f1 -->\n"
                    "<!-- brutal-pr-review-occurrence:old:abc123 -->\nold",
                    commit_id="abc123",
                )
            ],
        ) as env:
            ...
            self.assertIn("brutal-pr-review-occurrence:r1:abc123", posted_body_for(calls, "PATCH"))

    def test_multiple_existing_generated_summaries_do_not_block_new_summary(self):
        with fake_gh(
            issue_comments=[
                generated_comment(10, "<!-- brutal-pr-review:summary:v2:old -->\nold"),
                generated_comment(11, "<!-- brutal-pr-review:summary:v2:older -->\nolder"),
            ],
        ) as env:
    ...
    def test_existing_inline_from_old_head_creates_new_comment(self):
        with fake_gh(
            review_comments=[
                generated_comment(20, "<!-- brutal-pr-review:v2:f1 -->\nold", commit_id="oldsha")
            ],
        ) as env:
    ...
    def test_preserves_hidden_markers_in_rendered_bodies(self):
        with fake_gh() as env:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            planned = json.loads(result.stdout)
            self.assertIn("<!-- brutal-pr-review:summary:v2:r1 -->", planned["summary"]["body"])
            self.assertIn("<!-- brutal-pr-review:v2:f1 -->", planned["inline"][0]["body"])
            self.assertIn("<!-- brutal-pr-review-occurrence:r1:abc123 -->", planned["inline"][0]["body"])
            self.assertIn("Keep **markdown** intact.", planned["top_level"][0]["body"])
    ...
    def test_invalid_inline_line_falls_back_to_summary(self):
        with fake_gh() as env:
            ...
            self.assertEqual(planned["top_level"][0]["action"], "create_top_level")
            self.assertIn("line is not present in the current PR diff", planned["top_level"][0]["body"])
    ...
    def test_old_and_current_inline_markers_patch_current_comment(self):
        with fake_gh(
            review_comments=[
                generated_comment(20, "<!-- brutal-pr-review:v2:f1 -->\nold", commit_id="oldsha"),
                generated_comment(21, "<!-- brutal-pr-review:v2:f1 -->\ncurrent", commit_id="abc123"),
            ]
        ) as env:
            result = run_script(base_payload(), env, "--dry-run")
            self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_head_change_between_writes_stops_remaining_actions(self):
        with fake_gh(head_shas=["abc123", "abc123", "def456"]) as env:
            payload = base_payload(
                findings=[inline_finding("f1"), inline_finding("f2")]
            )
            result = run_script(payload, env)
            self.assertEqual(result.returncode, 2)
            self.assertIn("PR head changed", result.stderr)
            flat = [" ".join(call) for call in read_calls(env)]
            self.assertEqual(
                sum("POST repos/O/R/pulls/1/comments" in call for call in flat),
                1,
            )
            self.assertFalse(any("POST repos/O/R/issues/1/comments" in call for call in flat))
    ...
    def test_rejects_unsafe_review_id(self):
        with fake_gh() as env:
            payload = base_payload()
            payload["review_id"] = "bad-->id"
            result = run_script(payload, env, "--dry-run")
            self.assertEqual(result.returncode, 2)
            self.assertIn("review_id contains unsafe marker characters", result.stderr)

    def test_clean_review_posts_zero_finding_summary(self):
        with fake_gh() as env:
            result = run_script(base_payload(findings=[]), env, "--dry-run")
            self.assertEqual(result.returncode, 0, result.stderr)
            planned = json.loads(result.stdout)
            self.assertEqual(planned["validated_finding_count"], 0)
            self.assertEqual(planned["inline"], [])
            self.assertEqual(planned["top_level"], [])
            self.assertIn("Validated findings: 0", planned["summary"]["body"])

    def test_recurring_top_level_finding_patches_with_new_occurrence(self):
        with fake_gh(
            issue_comments=[
                generated_comment(
                    30,
                    "<!-- brutal-pr-review:v2:architecture -->\n"
                    "<!-- brutal-pr-review-occurrence:old:abc123 -->\nold",
                )
            ]
        ) as env:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            planned = json.loads(result.stdout)
            self.assertEqual(planned["top_level"][0]["action"], "patch_top_level")
            self.assertEqual(planned["top_level"][0]["comment_id"], 30)
            self.assertIn("brutal-pr-review-occurrence:r1:abc123", planned["top_level"][0]["body"])

    def test_legacy_finding_is_not_patched(self):
        with fake_gh(
            review_comments=[
                generated_comment(20, "<!-- gh-brutal-pr-review:f1 -->\nlegacy", commit_id="abc123")
            ]
        ) as env:
            result = run_script(base_payload(), env, "--dry-run")
            self.assertEqual(result.returncode, 0, result.stderr)
...
def base_payload(findings=None):
    return {
        ...
        "findings": [inline_finding("f1")] if findings is None else findings,
    }


def inline_finding(fingerprint, *, line=42, severity="CRITICAL"):
    return {
...
def posted_body_for(calls, method):
    for call in calls:
