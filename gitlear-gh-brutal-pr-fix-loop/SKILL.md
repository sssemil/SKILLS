---
name: gitlear-gh-brutal-pr-fix-loop
description: Loop Gitlear-aware GitHub PR brutal reviews and generated finding fixes. Use when Codex is asked to run repeated gitlear-gh-brutal-pr-review plus gitlear-gh-brutal-pr-finding-fixer passes, fix all generated PR findings, rerun review after fixes, polish Rust code idiomatically, trim fluff, reduce needless optionality, or continue until no review findings remain.
---

# Gitlear GH Brutal PR Fix Loop

Run a strict review/fix loop for a GitHub PR tied to Gitlear context. This skill
...
- `/home/user/Workspaces/SKILLS/gitlear-gh-brutal-pr-review/SKILL.md`
- `/home/user/Workspaces/SKILLS/gitlear-gh-brutal-pr-finding-fixer/SKILL.md`
...
- Respect source-skill safety stops for dirty worktrees, stale PR heads,
  fork/external PRs, incomplete thread state, push rejection, Gitlear ambiguity,
  and unavailable required tooling.
- Require Gitlear context only for review passes. Draining already-generated
  GitHub findings through the fixer remains GitHub-only.

## Workflow

### Step 1: Initialize The Loop

Resolve the target PR using `gitlear-gh-brutal-pr-review` rules. Record:

- PR number, URL, branch, base branch, and head SHA
- Gitlear issue display key
...
Run `gitlear-gh-brutal-pr-review` exactly as specified, with these additions:
...
Run `gitlear-gh-brutal-pr-finding-fixer` in continuous all-severities mode.
...
After the fixer stops, re-fetch PR state through the source-skill rules.
...
- PR, Gitlear issue, final head SHA, and stop reason
